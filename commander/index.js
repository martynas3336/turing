...
Command.prototype.executeSubCommand = function(argv, args, unknown, executableFile) {
  ...
  proc.on('error', function(err) {
    if (err.code === 'ENOENT') {
      throw new Error(`error: ${bin}(1) does not exist, try --help`)
      // console.error('error: %s(1) does not exist, try --help', bin);
    } else if (err.code === 'EACCES') {
      throw new Error(`error: ${bin}(1) not executable. try chmod or run with root`)
...
Command.prototype.missingArgument = function(name) {
  throw new Error(`error: missing required argument '${name}'`)
...
Command.prototype.optionMissingArgument = function(option, flag) {
  if (flag) {
    throw new Error(`error: option '${option.flags}' argument missing, got '${flag}'`)
...
Command.prototype.unknownOption = function(flag) {
  ...
  throw new Error(`error: unknown option '${flag}'`)
...
Command.prototype.variadicArgNotLast = function(name) {
  throw new Error(`error: variadic arguments must be last '${name}'`)
